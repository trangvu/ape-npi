...
#SBATCH --output=12k-en-de-%j.out
#SBATCH --error=12k-en-de-%j.err
...
APE_DATA_DIR="/home/xvuthith/da33/trang/dataset/en_de/APE16_LIG"
...
WK_DIR="/home/xvuthith/da33/trang/jobs/experiment/12k-end2end-"$DATE
mkdir -p $WK_DIR

echo "STEP 1: Train MT + AG"
MT_AG_CONFIG_FILE="config-MT-AG-en-de-12k.yaml"
cp $APE_CONF_PATH/origin-emb.yaml $WK_DIR/$MT_AG_CONFIG_FILE
...
MT_AG_LM_CONFIG_FILE="config-MT-AG-LM-en-de-12k.yaml"
RNN_MODEL_DIR=$WK_DIR"/lm-mt/model"
RNN_CELL_NAME="Model/RNN/multi_rnn_cell/cell_0/basic_lstm_cell"
cp $APE_CONF_PATH/chained.yaml $WK_DIR/$MT_AG_LM_CONFIG_FILE
...
#sed -i "/embedding_file:/c\    embedding_file: $MT_EMB_PATH" $WK_DIR/$MT_AG_LM_CONFIG_FILE
